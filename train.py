...
def create_model(n_var, n_first_layer, n_hidden_layers, n_last_layer, drop_out):
    model = tf.keras.Sequential()
    model.add(tf.keras.layers.Dense(n_first_layer, input_dim=n_var, activation='relu'))
    for n in n_hidden_layers:
        model.add(tf.keras.layers.Dense(n, activation='relu'))
    model.add(tf.keras.layers.Dropout(drop_out))
    model.add(tf.keras.layers.Dense(n_last_layer, activation='relu'))
    print(model.summary())
    return model
...
if __name__ == '__main__':
    ...
    model = create_model(n_var=trainData["data"].shape[1], n_first_layer=600, n_hidden_layers=[60,60,60], n_last_layer=1, drop_out=0.4)
